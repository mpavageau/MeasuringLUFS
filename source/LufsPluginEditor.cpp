...
LufsPluginEditor::~LufsPluginEditor()
{
    DEBUGPLUGIN_output("LufsPluginEditor::~LufsPluginEditor");
}
...
{
    DEBUGPLUGIN_output("LufsPluginEditor::resized width %d height %d", getWidth(), getHeight());
...
void LufsPluginEditor::timerCallback()
{
    if ( m_internallyPaused )
        return;

    //DEBUGPLUGIN_output("LufsPluginEditor::timerCallback");
...
{
    DEBUGPLUGIN_output("LufsPluginEditor::buttonClicked");
    LufsAudioProcessor* processor = getProcessor();
